...
if (instance_position(mouse_x, mouse_y, self)) && (image_xscale > 2.66){
	...
	if (mouse_check_button_pressed(mb_left)) && !(clicked){
		instance_create_layer(x, y, "WinIconFX", obj_window_icons, {image_xscale: image_xscale, image_yscale: image_yscale, image_index: entry + image_number, entry: entry, effect: true, menu: menu});
		clicked = true;
	};
}
	...
	if (image_alpha <= 0){
		switch entry{
			case 0:
				...
				break;
			case 1: 
				if (menu == 0) instance_create_layer(room_width / 2, room_height / 2, "Indexplorer", obj_indexplorer);
				break;
		};
	};
};
